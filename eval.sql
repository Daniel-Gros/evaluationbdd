-- Active: 1712153234224@@127.0.0.1@3306@phpmyadmin
...
GRANT INSERT, UPDATE, DELETE ON cinema_db.Seance TO 'user1'@'localhost';

GRANT INSERT, UPDATE, DELETE ON cinema_db.Seance TO 'user2'@'localhost';

GRANT INSERT, UPDATE, DELETE ON cinema_db.Seance TO 'user3'@'localhost';

-- Suppression des comptes suite quiproquos entre 2 formules de requêtes --
DELETE FROM Administrateur WHERE idAdmin = 1;
DELETE FROM Utilisateur WHERE idUtilisateur = 1;
DELETE FROM Utilisateur WHERE idUtilisateur = 2;
DELETE FROM Utilisateur WHERE idUtilisateur = 3;

REVOKE ALL PRIVILEGES ON cinema_db.* FROM 'admin'@'localhost';
REVOKE INSERT, UPDATE, DELETE ON cinema_db.Seance FROM 'user1'@'localhost';
REVOKE INSERT, UPDATE, DELETE ON cinema_db.Seance FROM 'user2'@'localhost'; 
REVOKE INSERT, UPDATE, DELETE ON cinema_db.Seance FROM 'user3'@'localhost';

-- Correction et création des comptes administrateur et utilisateur -- COMMENT
...
VALUES 
    ('Cinéma 1', '289 Boulevard des Belges', 'Lyon', 4, 2),
    ('Cinéma 2', 'Place du Prado', 'Marseille', 5, 2),
    ('Cinéma 3', 'Bercy Village', 'Paris', 6, 2);